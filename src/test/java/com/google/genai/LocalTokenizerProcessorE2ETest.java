...
public class LocalTokenizerProcessorE2ETest {
    ...
    void testEncodeIDs(String inputText, List<Integer> expectedTokens) throws Exception {
        ...
        assertEquals(
                expectedTokens, actualPieces.stream().map(Token::id).collect(Collectors.toList()),
    ...
    static Stream<Arguments> encodeIDsTestCases() {
        return Stream.of(
                ...
                Arguments.of("Language: English\r\n\r\nCredits: Produced by David Widger\r\n", Arrays.asList(14357, 235292, 4645, 235316, 108, 235316, 108, 34711, 235292, 99662, 731, 6046, 37303, 1197, 235316, 108)),
                Arguments.of("Bienvenido a este proyecto", Arrays.asList(176831, 476, 4004, 25431)),
                Arguments.of("अस्मिन् परियोजनायां स्वागतम्", Arrays.asList(236088, 22740, 212361, 18029, 14480, 19900, 146166, 6751, 235563, 56545, 44071, 235550, 26989)),
                Arguments.of("if allow == true { return x;} else {return x+y;}", Arrays.asList(648, 2765, 1159, 1382, 612, 2203, 1141, 22505, 1354, 612, 773, 1141, 235340, 235267, 22505))
        );
    }
    ...
    void testEncodeTexts(String inputText, List<Token> expectedTokens) throws Exception {
        ...
        assertEquals(expectedTokens, actualTokens, String.format("Encoding '%s' did not produce the expected tokens", inputText));
    }

    static Stream<Arguments> encodeTextsTestCases() {
        return Stream.of(
                Arguments.of("hi <td> bye", ImmutableList.of(
                        new Token("hi", 544),
                        new Token("▁", 235248),
                        new Token("<td>", 176),
                        new Token("▁bye", 44788)
                )),
                Arguments.of("hiƻ <td>🤨there ⇲bob, สวัสดี", ImmutableList.of(
                        new Token("hi", 544),
                        new Token("<0xC6>", 415),
                        new Token("<0xBB>", 404),
                        new Token("▁", 235248),
    ...
    void testDecodeIds(List<Integer> ids, String expectedText) {
        ...
        assertEquals(expectedText, actualText, String.format("Decoding IDs %s did not produce the expected text", ids));
    }

    static Stream<Arguments> decodeIdsTestCases() {
        return Stream.of(
                Arguments.of(Arrays.asList(17534, 2134), "hello world"),
                Arguments.of(Arrays.asList(427, 365, 428, 357, 29422, 1653, 427, 365, 428, 357), "Ҕӌnever againҔӌ"),
                Arguments.of(Arrays.asList(785, 2017, 108, 639, 2550, 2017), "one line\nand another line"),
                Arguments.of(Arrays.asList(1001, 1002, 1003, 1004), "buark}) res"),
                Arguments.of(Arrays.asList(111001, 111002, 111003, 111004), " Wichita EducaçãoVocabulary天堂"),
                Arguments.of(Arrays.asList(139), "  "),
                Arguments.of(Arrays.asList(140), "   "),
                Arguments.of(Arrays.asList(145), "        "),
