...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw GeminiNextGenApiInvalidDataException("`$name` is not set")
            is JsonNull -> throw GeminiNextGenApiInvalidDataException("`$name` is null")
            else -> throw GeminiNextGenApiInvalidDataException("`$name` is invalid, received $this")
        }
