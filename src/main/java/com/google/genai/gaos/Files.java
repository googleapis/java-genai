...
public class Files {
    ...
    public UploadEnvironmentFileResponse upload(
        ...
        try {
            if (sizeBytes == 0) {
                ...
                try {
                    ...
                    while (offset < sizeBytes) {
                        ...
                        String command = isFinal ? "upload, finalize" : "upload";

                        byte[] chunkBytes = Arrays.copyOf(buffer, bytesRead);
                        HttpRequest chunkReq = HttpRequest.builder()
                                .method("POST")
                                .uri(URI.create(uploadUrl))
                                .setHeader("X-Goog-Upload-Command", command)
                                .setHeader("X-Goog-Upload-Offset", String.valueOf(offset))
                                .setHeader("user-agent", SDKConfiguration.USER_AGENT)
                                .body(HttpBody.of(chunkBytes))
                                .build();
                        uploadResponse = this.sdkConfiguration.client().send(chunkReq);
                        if (uploadResponse.statusCode() >= 400) {
                            throw GaosApiException.from("Upload chunk failed with status: " + uploadResponse.statusCode(), uploadResponse);
                        }
                        offset = nextOffset;
            ...
            throw new RuntimeException("Failed during upload chunk transfer", e);
        }
        ...
        try {
            ...
            if (node != null && node.isObject()) {
                if (node.has("files")) {
                    ...
                    files = new GetEnvironmentFilesResponse(Arrays.asList(file), null);
