...
public class Reflections {
  ...
  /**
   * Extracts the underlying value from an enum wrapper instance if the class follows the enum
   * wrapper pattern.
   *
   * <p>An enum wrapper is a class that emulates enum behavior but can handle unknown values without
   * runtime errors. This pattern is commonly used for API responses where new enum values might be
   * added over time.
   *
   * <p>The method validates that the class follows the enum wrapper pattern by checking for:
   *
   * <ul>
   *   <li>A static factory method {@code of(String)} or {@code of(Integer)} that returns the class
   *       type
   *   <li>An instance method {@code value()} returning String or Integer
   *   <li>At least one public static final field of the same class type (predefined constants)
   * </ul>
   *
   * <p>If all validation passes, the method invokes the {@code value()} method on the provided
   ...
   *     follows the enum wrapper pattern and the value extraction succeeds, {@code
