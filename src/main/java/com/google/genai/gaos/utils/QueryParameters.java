...
public class QueryParameters {
  public static <T extends Object> List<QueryParameter> parseQueryParams(
    ...
    for (Field field : fields) {
      ...
      if (queryParamsMetadata.serialization != null
          && !Java8Compat.isBlank(queryParamsMetadata.serialization)) {
        ...
        switch (queryParamsMetadata.style) {
          case "form":
            ...
            break;
          case "deepObject":
            List<QueryParameter> deepObjectParams =
                parseDeepObjectParams(queryParamsMetadata, value);
            allParams.addAll(deepObjectParams);
            break;
          case "pipeDelimited":
            List<QueryParameter> pipeDelimitedParams =
                parseDelimitedParams(queryParamsMetadata, value, "|");
    ...
    if (globals != null) {
      ...
      globals
          ...
          .forEach(
              entry -> allParams.add(QueryParameter.of(entry.getKey(), entry.getValue(), false)));
    }

    return allParams;
  }

  private static List<QueryParameter> parseSerializedParams(
      QueryParamsMetadata queryParamsMetadata, Object value) throws JsonProcessingException {
    ...
    switch (queryParamsMetadata.serialization) {
      case "json":
        ...
        params.add(
            QueryParameter.of(queryParamsMetadata.name, json, queryParamsMetadata.allowReserved));
        break;
      default:
        break;
    }
  ...
  private static List<QueryParameter> parseDelimitedParams(
      QueryParamsMetadata queryParamsMetadata, Object value, String delimiter)
          ...
          params.addAll(
              values.stream()
                  .map(
                      v ->
                          QueryParameter.of(
                              queryParamsMetadata.name, v, queryParamsMetadata.allowReserved))
      ...
      case OBJECT:
        {
          ...
          for (Field field : fields) {
            ...
            if (queryParamsMetadata.explode) {
              params.add(
                  QueryParameter.of(metadata.name, Utils.valToString(val), metadata.allowReserved));
            } else {
  ...
  private static List<QueryParameter> parseDeepObjectParams(
      QueryParamsMetadata queryParamsMetadata, Object value) throws Exception {
