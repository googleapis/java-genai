 ...
 *
 * <p>Each SSE message's {@code data} field is deserialized into the type {@code T}, allowing for
 * easy processing of events as domain objects.
 *
 * <h2>Event Consumption</h2>
 *
 * <p>Events can be consumed in multiple ways:
 *
 * <ul>
 *   <li><b>Iteration:</b> Use a for-each loop to process each event:
 * </ul>
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li><b>Stream API:</b> Consume events as a Java Stream (must be closed after use):
 * </ul>
 *
 * <pre>{@code
 * try (EventStream<MyEvent> eventStream = new EventStream<>(...);
 ...
 *
 * <ul>
 *   <li><b>Collect to List:</b> Read all remaining events into a list:
 * </ul>
 *
 * <pre>{@code
 * try (EventStream<MyEvent> eventStream = new EventStream<>(...)) {
 ...
 * }</pre>
 *
 * <p>Events are lazily loaded from the underlying SSE stream. Consumption stops either when the
 * stream ends or when an optional terminal message is encountered.
 *
 * <p><b>Important:</b> This class implements {@link AutoCloseable} and must be used within a
 * <em>try-with-resources</em> block to ensure that underlying streams are properly closed after
 * consumption, preventing resource leaks.
 *
...
public final class EventStream<T> implements Iterable<T>, AutoCloseable {
  ...
  public Optional<T> next() throws IOException {
    while (!terminated) {
      ...
      boolean isTerminal =
          terminalMessage.flatMap(sentinel -> msg.data().map(sentinel::equals)).orElse(false);
      if (isTerminal) {
        terminated = true;
      ...
      if (dataRequired && !msg.data().isPresent()) {
        if (logger.isTraceEnabled()) {
  ...
  /**
   * Reads all events and returns them as a {@code List}. This method calls {@code close()}.
   *
