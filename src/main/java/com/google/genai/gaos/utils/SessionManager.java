...
public final class SessionManager<T extends SessionManager.HasSessionKey> {
  ...
  public Session<T> getSession(
      T credentials, List<String> scopes, Function<List<String>, Session<T>> tokenProvider) {
  ...
  /**
   * Checks if the token has expired. If no expires_in field was returned by the authorization
   * server, the token is considered to never expire. A buffer (REFRESH_BEFORE_EXPIRY_SECONDS) is
   * applied to refresh tokens before they actually expire.
   */
  // VisibleForTesting
