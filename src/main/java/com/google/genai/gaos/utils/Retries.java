...
public class Retries {
  ...
  public HttpResponse<InputStream> run() throws Exception {
    ...
    switch (retryConfig.strategy()) {
      ...
      case ATTEMPT_COUNT_BACKOFF:
        ...
        return retryWithAttemptCountBackoff(
            backoff.retryConnectError(), backoff.retryReadTimeoutError());
  ...
  private HttpResponse<InputStream> getResponse(
      boolean retryConnectError, boolean retryReadTimeoutError, int attempt) throws Exception {
    try {
  ...
  private HttpResponse<InputStream> retryWithBackoff(
    ...
    while (true) {
      try {
        ...
        logger.debug(
            "Non-retryable exception encountered: {}", e.exception().getClass().getSimpleName());
        ...
        if (logger.isTraceEnabled()) {
          ...
          logger.trace(
              "Retrying due to {} - waiting {}ms before attempt {}",
              reason,
              sleepMs,
              numAttempts + 1);
        }
