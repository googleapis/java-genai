...
import com.google.genai.gaos.utils.Hook.AfterError;
import com.google.genai.gaos.utils.Hook.AfterErrorContext;
import com.google.genai.gaos.utils.Hook.AfterSuccess;
import com.google.genai.gaos.utils.Hook.AfterSuccessContext;
import com.google.genai.gaos.utils.Hook.BeforeRequest;
import com.google.genai.gaos.utils.Hook.BeforeRequestContext;
import com.google.genai.gaos.utils.Hook.SdkInit;
...
/**
 * Registers hooks for use at runtime by an end-user or for use by a customer that may edit the
 * SDKHooks.java file.
 *
 * <p>For example, this code will add a transaction id header to every request:
 *
 * <pre>
 * hooks.registerBeforeRequest((context, request) -&gt; {
 *     return request.toBuilder()
 *             .header("acme-transaction-id", nextTransactionId())
 *             .build();
 * });
 * </pre>
 */
...
public class Hooks implements BeforeRequest, AfterSuccess, AfterError, SdkInit {
  ...
  /**
   * Registers a {@link BeforeRequest} hook to be applied in order of registration. The result of
   * the first BeforeRequest hook will be passed to the second BeforeRequest hook and processed
   * similarly for the rest of the registered hooks. If a BeforeRequest hook throws then that
  ...
  /**
   * Registers an {@link AfterSuccess} hook to be applied in order of registration (multiple can be
   * registered). The result of the first AfterSuccess hook will be passed to the second
   * AfterSuccess hook and processed similarly for the rest of the registered hooks. If an
   * AfterSuccess hook throws then that Exception will <b>not</b> be passed to the {@link
   * AfterError} hooks.
   *
  ...
  /**
   * Registers an {@link AfterError} hook to be applied in order of registration (multiple can be
   * registered). If the first AfterError hook throws then the second hook will be called with that
   * exception (and no response object) and so on for the rest of the registered hooks. If an
   * AfterError hook returns normally then its result will be passed through to the next AfterError
   * hook with the latest thrown Exception.
   *
  ...
  /**
   * Registers a {@link SdkInit} hook to be applied in order of registration (multiple can be
   * registered).
   *
  ...
  public Hooks registerSdkInit(SdkInit SdkInit) {
    ...
    logger.debug(
        "Registered SdkInit hook: {} (total: {})",
        SdkInit.getClass().getSimpleName(),
        SdkInitHooks.size());
    return this;
  }
  ...
  @Override
  public HttpResponse<InputStream> afterSuccess(
      AfterSuccessContext context, HttpResponse<InputStream> response) throws Exception {
    Utils.checkNotNull(context, "context");
  ...
  public HttpResponse<InputStream> afterError(
    ...
    if (logger.isTraceEnabled() && !afterErrorHooks.isEmpty()) {
      logger.trace(
          "Executing {} afterError hook(s) for operation: {}",
          afterErrorHooks.size(),
          context.operationId());
    }
