...
/**
 * Generic streaming parser that handles byte buffer management and delegates format-specific logic
 * to a StreamContentProcessor.
 */
public final class StreamingParser<T> {
  ...
  public Optional<T> finish() {
    if (byteBuffer.position() > 0) {
      ...
      byteBuffer.clear();
      String content =
          processor.sanitizeContent(new String(remainingBytes, StandardCharsets.UTF_8), first);
  ...
  private Optional<T> extractNextFromBytes() {
    ...
    byteBuffer.compact();
    String content =
        processor.sanitizeContent(new String(contentBytes, StandardCharsets.UTF_8), first);
    if (first) {
      first = false;
    }
