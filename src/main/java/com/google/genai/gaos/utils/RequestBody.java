...
public final class RequestBody {
  ...
  private static void serializeMultipartFile(
    ...
    for (Field field : fields) {
      ...
      MultipartFormMetadata metadata = MultipartFormMetadata.parse(field);
      if (metadata == null
          || (!metadata.content && (metadata.name == null || Java8Compat.isBlank(metadata.name)))) {
        continue;
      }
  ...
  public static SerializedBody serializeFormData(Object value)
              ...
              case OBJECT:
                {
                  ...
                  Optional<?> unwrappedEnumValue =
                      Reflections.getUnwrappedEnumValue(val.getClass(), val);
                  if (unwrappedEnumValue.isPresent()) {
                    params.add(
                        new NameValue(metadata.name, Utils.valToString(unwrappedEnumValue.get())));
                    break;
                  }
    ...
    return new SerializedBody(
        contentType, HttpBody.of(query.getBytes(StandardCharsets.ISO_8859_1)));
  }
}
