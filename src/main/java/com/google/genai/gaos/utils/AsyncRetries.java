...
public class AsyncRetries {
  ...
  private <T> void attempt(
    ...
    CompletableFuture<HttpResponse<InputStream>> attemptFuture;
    try {
  ...
  private boolean shouldRetryIOException(Throwable e, BackoffStrategy backoff) {
    ...
    return (isConnectTimeout(e) && backoff.retryConnectError())
        || (isReadTimeout(e) && backoff.retryReadTimeoutError());
  }
  ...
  private RetryDecision shouldRetry(HttpResponse<InputStream> response) {
    boolean matched =
        retriableStatusCodes.stream()
  ...
  private void maybeRetry(
    ...
    if (retryConfig.strategy() == RetryConfig.Strategy.BACKOFF
      ...
      // retry exhausted
      logger.debug(
          "Async retry exhausted after {}ms, {} attempts",
          timeSinceStart.toMillis(),
          state.count() + 1);
      if (e instanceof AsyncRetryableException) {
        result.complete(((AsyncRetryableException) e).response());
        return;
    ...
    if (logger.isTraceEnabled()) {
      String reason =
          e instanceof AsyncRetryableException
              ? "status " + ((AsyncRetryableException) e).response().statusCode()
              : e.getClass().getSimpleName();
      logger.trace(
          "Async retrying due to {} - waiting {}ms before attempt {}",
          reason,
          intervalMs,
          state.count() + 1);
    }
    ...
    if (e instanceof AsyncRetryableException) {
      closeQuietly(((AsyncRetryableException) e).response());
    }

    if (scheduler == null) {
      result.completeExceptionally(
          new IllegalStateException(
              "Async retry scheduler is not configured. Please provide a"
  ...
  public static final class Builder {
    ...
    /**
     * Defines the scheduler that will be used to schedule and execute retry attempts. Recommend
     * using a globally shared executor for this.
     *
