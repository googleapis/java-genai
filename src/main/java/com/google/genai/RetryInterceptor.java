  ...
  public Response intercept(Chain chain) throws IOException {
    ...
    while (true) {
      ...
      try {
        ...
        // If the response is successful or the response code is not in the retry list, return the
        // response.
        if (response.isSuccessful() || !httpStatusCodes.contains(response.code())) {
          return response;
        }
        // If the max attempts is reached, return the response.
        ...
        // Close the response if it's not the last attempt, so that the connection can be reused.
        if (response != null && attempt < maxAttempts) {
          response.close();
        }
      }
