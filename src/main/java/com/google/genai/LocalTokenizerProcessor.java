...
// IMPORTANT: The BPE algorithm end to end test is disabled in mvn test. 
// If you change the code in this file, remember to enable the test
// and run `mvn test -Dtest=com.google.genai.LocalTokenizerProcessorE2ETest`
...
class Trie {
    ...
    public boolean search(String word) {
        ...
        for (i = 0; i < word.length(); i++) {
            ...
            if (node == null)
                return false;
            if (i == word.length() - 1 && node.freq > 0)
                return true;
        }
        return false;
    }
