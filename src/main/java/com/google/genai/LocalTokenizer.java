 ...
 * <li>Forward compatibility depends on the open-source tokenizer models for
 * future Gemini
 * versions.
 * </ul>
 *
 * <p>
 * <b>NOTE:</b> The SDK's local tokenizer implementation is experimental and may
 * change in the
...
public final class LocalTokenizer {
    ...
    public LocalTokenizer(String modelName) {
        ...
        try {
            ...
            throw new RuntimeException("Failed to initialize LocalTokenizerProcessor", e);
        }
    }
     ...
     * @return A {@link CountTokensResponse} containing the total number of tokens.
     */
    public CountTokensResponse countTokens(List<Content> contents, CountTokensConfig config) {
        ...
        return CountTokensResponse.builder().totalTokens(totalTokens).build();
    }

    public CountTokensResponse countTokens(Content content, CountTokensConfig config) {
        return countTokens(ImmutableList.of(content), config);
    }

    public CountTokensResponse countTokens(String content, CountTokensConfig config) {
     ...
     *
     * @param contents The contents to tokenize.
     * @return A {@link ComputeTokensResponse} containing the token information.
     */
    public ComputeTokensResponse computeTokens(List<Content> contents) {
        List<Content> processedContents = contents;
        ...
        return ComputeTokensResponse.builder().tokensInfo(tokenInfos).build();
    }

    public ComputeTokensResponse computeTokens(Content content) {
        return computeTokens(ImmutableList.of(content));
    }

    public ComputeTokensResponse computeTokens(String content) {
     ...
     *
     * <p>
     * This class is responsible for traversing complex {@link Content} and
     * {@link Tool} objects
     * and extracting all the text content that should be included when calculating
     * token counts.
     *
     * <p>
     * A key feature of this class is its ability to detect unsupported fields in
     * {@link Content}
     * objects. If a user provides a {@link Content} object with fields that this
     * local tokenizer
     * doesn't recognize (e.g., new fields added in a future API update), this class
     * will log a
     * warning.
     *
     * <p>
     * The detection mechanism for {@link Content} objects works by recursively
     * building a
     * "counted" version of the input object. This "counted" object only contains
     * the data that was
     * successfully processed and added to the text list for tokenization. After
     * traversing the
     * input, the original {@link Content} object is compared to the "counted"
     * object. If they don't
     * match, it signifies the presence of unsupported fields, and a warning is
     * logged.
     */
    private static class TextsAccumulator {
        ...
        private Content addContentAndBuildCounted(Content content) {
            ...
            if (content.parts().isPresent()) {
                List<Part> countedParts = content.parts().get().stream()
