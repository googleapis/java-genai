...
public final class TuningJobsAsync {
  ...
  public static void main(String[] args) {
    ...
    if (client.vertexAI()) {
      ...
      tuningJob1Future
          .thenCompose(
              tuningJob1 -> {
                ...
                CompletableFuture<TuningJob> tuningJob2Future =
                    client.async.tunings.get(tuningJob1.name().get(), null);

                return tuningJob2Future.thenAccept(
                    tuningJob2 -> {
                      ...
                      while (tunedModel.isEmpty()) {
                        ...
                        // Get the tuning job.
                        TuningJob fetchedTuningJob =
                            client.tunings.get(tuningJob1.name().get(), null);
      ...
      // Tuning jobs creation is not supported in Gemini Developer API.
    }
    ...
    asyncPagerFuture
        ...
        .thenAccept(
            page -> {
              page.forEach(job -> System.out.println(job.name().get() + "\n" + job.state().get()));
            })
        .join();
  }
