...
public final class InteractionFunctionCallingServerState {
  public static void main(String[] args) {
    ...
    if (interaction.steps().isPresent()) {
      List<Step> steps = interaction.steps().get();
      for (Step step : steps) {
        if (step instanceof FunctionCallStep) {
          FunctionCallStep functionCall = (FunctionCallStep) step;
          functionCallId = functionCall.id().orElse(null);
          functionName = functionCall.name().orElse(null);
          ...
          if (outputStep.content().isPresent()) {
            for (Content output : outputStep.content().get()) {
              if (output instanceof TextContent) {
                ((TextContent) output).text().ifPresent(text -> System.out.println("Output Text: " + text));
              }
            }
          }
        }
    ...
    if (functionCallId != null) {
      ...
      String id = interaction.id().orElseThrow(() -> new RuntimeException("No ID returned"));

      CreateModelInteraction followUpParams =
          CreateModelInteraction.builder()
              .model(Model.of(Constants.GEMINI_MODEL_NAME))
              .previousInteractionId(id)
              .input(InteractionsInput.ofStep(Collections.singletonList(functionResult)))
              .build();

      CreateInteractionResponse followUpResponse =
          client.interactions.create(CreateInteractionRequestBody.of(followUpParams));
      Interaction followUpInteraction =
          followUpResponse.interaction().orElseThrow(() -> new RuntimeException("No interaction returned"));

      System.out.println("Final response status: " + followUpInteraction.status().orElse(null));
      if (followUpInteraction.steps().isPresent()) {
        for (Step step : followUpInteraction.steps().get()) {
          if (step instanceof ModelOutputStep) {
            ModelOutputStep outputStep = (ModelOutputStep) step;
            if (outputStep.content().isPresent()) {
              for (Content output : outputStep.content().get()) {
                if (output instanceof TextContent) {
                  ((TextContent) output).text().ifPresent(text -> System.out.println("Final Output: " + text));
                }
              }
            }
          }
