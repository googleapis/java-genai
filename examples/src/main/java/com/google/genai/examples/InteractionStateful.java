 ...
 *
 * <p>mvn clean compile exec:java -Dexec.mainClass="com.google.genai.examples.InteractionStateful"
 */
...
public final class InteractionStateful {
  public static void main(String[] args) {
    ...
    Interaction interaction1 =
        client
            .interactions
            .create(CreateInteractionRequestBody.of(params1))
            .interaction()
            .orElseThrow(() -> new RuntimeException("No interaction returned"));
    System.out.println("Model: ");
    printOutput(interaction1);

    String id1 = interaction1.id().orElseThrow(() -> new RuntimeException("No ID returned"));
          ...
          if (outputStep.content().isPresent()) {
            for (Content content : outputStep.content().get()) {
              if (content instanceof TextContent) {
                ((TextContent) content).text().ifPresent(text -> System.out.println(text));
              }
            }
          }
        }
